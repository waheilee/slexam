...
function checkType($num){
    $difys = array('1'=>'简单','中等','困难');
    return $difys[$num];
}

function checkGb($num){
    if($num >= 130){
        return '优';
...
function getName($num){
    $array = array(1=>'真题试卷','模拟题库','自定义模拟题库');
    return $array[$num];
}

function getBfb($num,$eNum){
    $sNum =  number_format((($num-$eNum)/$num),3) * 100;
    return $sNum;
}

//检测用户是否点赞过打榜
function checkLaud($info,$userid){
    if(strpos($info,','.$userid.',') !== false){
        return 'no';
    }else{
        return 'yes';
    }
...
//获取正确率
function getAccuracy($num,$count,$type)
{   
    $Accuracy = (($count - $num) / $count); 
    $data = number_format($Accuracy, 2, '.', '') * 100;
